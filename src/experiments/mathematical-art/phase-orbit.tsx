  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = iterations; i > 0; i -= 1) {
        ...
        const c = d / 2 - time / 3
        const x = (d * 19 + 29 + k * k) * Math.sin(c)
        const denominator = y < 9 ? 7 : 203 * Math.sin(e / 2)
        const pointY = 66 * Math.sin(c / 3) + 4 * Math.sin(k * 2) + d ** 3 / 3 * Math.sin(time * 3 - d * d / 4) + y / (denominator || 0.0001) * k * e
